...
| 1.7 | MEDIUM | Xoá/ẩn tin không đóng report OPEN → report kẹt vĩnh viễn, `openReports` lệch | `listing.service.ts:1177-1182,1292-1302`, `report.service.ts:114-120` | ✅ `reportRepository.resolveAllOpenForListings` (action `target_removed`) gọi từ chủ tin xoá, bàn duyệt gỡ, khoá tài khoản; `assertCanResolve` và `resolve` đọc `withDeleted` nên báo cáo mồ côi đóng được bằng `ignore`. Test `report-lifecycle` (7 ca) |
| 1.8 | MEDIUM | `GET /favorites` `findByIds` có scope không kẹp `status` → thấy tin hidden/rejected của org | `listing.service.ts:1039-1044`, `listing.repository.ts:216-218` | ✅ `findByIds` kẹp `PUBLIC_LISTING_STATUSES` (cùng luật với `getForViewer`) + trả cùng DTO `withOrgBadge` như bảng tin. Test `favorite` thêm 2 ca (ẩn/từ chối sau khi lưu → rơi khỏi danh sách, mở lại → về lại kèm badge) |
...
| 2.2 | MEDIUM | Race 2 moderator cùng đóng 1 report → trừ uy tín 2 lần | `report.service.ts:182-217` | ✅ `reportRepository.claimOpen` = CAS trên `status: open` TRƯỚC mọi side-effect, người thua nhận 409. Tin đọc SAU khi nhận xử nên ca hai báo cáo KHÁC NHAU cùng tin cũng chỉ trừ một lần (thấy `hidden` → không phạt, hoặc thua CAS của `setModerationStatus`). Test `report-race` (3 ca, người bán ở bậc 2 để phân biệt trừ 1 với trừ 2) |
...
| 3.2 | MEDIUM | `membershipService.remove` không thu hồi grant org/org_unit → người bị gỡ vẫn duyệt | `membership.service.ts:91-103`, `tenant.middleware.ts:153-156` | ✅ `membershipService.remove` → `revokeAllForUserInOrg` (org + org_unit). Test `admin-grant-membership` |
...
| 5.6 | MEDIUM | `Organization` có `deletedAt` nhưng không hook soft-delete; `allImageUrls` sót filter | `organization.model.ts:122`, `organization.repository.ts:180` | ✅ hook `excludeDeleted` ở model (`/^find/` + `countDocuments`, `withDeleted` để đọc cả bản đã xoá) — cùng khuôn OrgUnit/User/Listing. Test `organization-soft-delete` (4 ca, có `allImageUrls`) |
