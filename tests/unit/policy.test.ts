...
/**
 * `canApproveListing` chọn nhánh theo TRỤC CỦA TIN. Nó là chốt duy nhất đứng giữa
 ...
 */
describe('canApproveListing — trục của tin chọn người có thẩm quyền', () => {
  const publicInJobHcm = {
    reach: 'marketplace' as const,
    organizationId: null,
    unitId: null,
    categoryId: CAT_JOB,
    provinceCode: HCM,
  ...
  it('quyền org KHÔNG với tới tin trục danh mục', () => {
    expect(canApproveListing(orgManagerA, publicInJobHcm)).toBe(false)
  })

  it('quyền đúng ô (danh mục × tỉnh) thì với tới', () => {
    expect(canApproveListing(catManagerJobHcm, publicInJobHcm)).toBe(true)
  })

  it('sai danh mục hoặc sai tỉnh thì không', () => {
    expect(canApproveListing(catManagerJobHcm, { ...publicInJobHcm, categoryId: CAT_BOOK })).toBe(
      false,
    )
    expect(canApproveListing(catManagerJobHcm, { ...publicInJobHcm, provinceCode: HANOI })).toBe(
      false,
    )
  })

  it('ngược lại: quyền danh mục KHÔNG với tới tin nội bộ của org', () => {
    expect(canApproveListing(catManagerJobHcm, internalOfOrgA)).toBe(false)
    expect(canApproveListing(orgManagerA, internalOfOrgA)).toBe(true)
  })
  ...
  it('staff nhóm con chỉ duyệt được tin của nhóm mình', () => {
    ...
    expect(canApproveListing(staffUnit1, internalOfOrgA)).toBe(true)
    expect(canApproveListing(staffUnit1, { ...internalOfOrgA, unitId: UNIT_2 })).toBe(false)
    // Tin không ghi nhóm con phải đẩy lên manager org, không rơi vào tay staff bất kỳ.
    expect(canApproveListing(staffUnit1, { ...internalOfOrgA, unitId: null })).toBe(false)
  })
  ...
  it('tin công khai thiếu tỉnh: chỉ grant toàn quốc mới với tới', () => {
    ...
    expect(canApproveListing(catManagerJobHcm, noProvince)).toBe(false)
    expect(canApproveListing(nationwide, noProvince)).toBe(true)
  })
})

/**
 * `canTakedownListing` — cửa RỘNG hơn đúng một vế: nhóm sở hữu tin rút được tin mang tên mình
 * kể cả khi tin đó nằm trên trục danh mục.
 *
 * Đây là cặp đối chứng của khối trên. Cùng một grant, cùng một tin, hai câu trả lời khác nhau
 * — và chính sự khác nhau đó là toàn bộ luật "một cửa duyệt, hai cửa gỡ". Để hai khối cạnh nhau
 * để không ai sửa một bên mà quên bên kia.
 */
describe('canTakedownListing — nhóm gỡ được tin mang tên mình', () => {
  /** Tin CÔNG KHAI nhưng vẫn đeo badge nhóm A — ca duy nhất hai cửa trả lời khác nhau. */
  const publicBadgedA = {
    reach: 'marketplace' as const,
    organizationId: ORG_A,
    unitId: null,
  ...
  it('quản trị nhóm KHÔNG duyệt được nhưng GỠ được tin lên sàn mang tên nhóm', () => {
    expect(canApproveListing(orgManagerA, publicBadgedA)).toBe(false)
    expect(canTakedownListing(orgManagerA, publicBadgedA)).toBe(true)
  })

  it('tin lên sàn KHÔNG mang tên nhóm nào thì nhóm vẫn không đụng được', () => {
    const unbadged = { ...publicBadgedA, organizationId: null }
    expect(canTakedownListing(orgManagerA, unbadged)).toBe(false)
  })

  it('quản trị nhóm KHÁC không gỡ được', () => {
    const orgManagerB: Grant[] = [
      { role: SYSTEM_ROLES.MANAGER, scopeType: SCOPE_TYPES.ORG, orgId: ORG_B },
    ]
    expect(canTakedownListing(orgManagerB, publicBadgedA)).toBe(false)
  })

  /**
   * Staff nhóm con bị loại KHÔNG phải bằng một luật riêng, mà vì `routeListing` đặt
   * `unitId: null` cho mọi tin trục công khai và `sameId(null, null)` là `false`. Ghim lại để
   * ai đó "sửa" `sameId` cho nhận `null` sẽ làm đỏ đúng ở đây.
   */
  it('staff nhóm con KHÔNG gỡ được tin lên sàn của nhóm mình', () => {
    const staffUnit1: Grant[] = [
      { role: SYSTEM_ROLES.STAFF, scopeType: SCOPE_TYPES.ORG_UNIT, orgId: ORG_A, unitId: UNIT_1 },
    ]
    expect(canTakedownListing(staffUnit1, publicBadgedA)).toBe(false)
  })

  it('với tin NỘI BỘ hai cửa trùng khít — vế thêm không nới gì', () => {
    const internalOfOrgA = {
      reach: 'members' as const,
      organizationId: ORG_A,
      unitId: UNIT_1,
    ...
    expect(canTakedownListing(orgManagerA, internalOfOrgA)).toBe(
      canApproveListing(orgManagerA, internalOfOrgA),
    )
    expect(canTakedownListing(catManagerJobHcm, internalOfOrgA)).toBe(false)
  })

  it('người phụ trách danh mục vẫn gỡ được tin trong ô của mình', () => {
    const catManagerJobHcm: Grant[] = [
      {
        role: SYSTEM_ROLES.MANAGER,
        scopeType: SCOPE_TYPES.CATEGORY_PROVINCE,
    ...
    expect(canTakedownListing(catManagerJobHcm, publicBadgedA)).toBe(true)
  })
})

/**
 * `canApproveListing` ở tầng phường: trục của TIN vẫn là thứ chọn người có thẩm quyền, chỉ khác
 * là ô của tin giờ hẹp tới phường.
 */
describe('canApproveListing — tầng phường', () => {
  const publicInLagi = {
    reach: 'marketplace' as const,
    organizationId: null,
    unitId: null,
  ...
  it('quyền đúng phường thì với tới, sai phường thì không', () => {
    expect(canApproveListing([catManagerWards], publicInLagi)).toBe(true)
    expect(canApproveListing([catManagerWards], { ...publicInLagi, wardCode: WARD_DALAT })).toBe(
      false,
    )
  })

  it('quyền cấp tỉnh vẫn phủ tin của mọi phường trong tỉnh', () => {
    expect(canApproveListing([catManagerLamDong], publicInLagi)).toBe(true)
    expect(canApproveListing([catManagerLamDong], { ...publicInLagi, wardCode: WARD_DALAT })).toBe(
      true,
    )
  })
