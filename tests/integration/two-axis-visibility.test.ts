 ...
 * | organizationId | reach        | thành viên org | người ngoài |
 * |----------------|--------------|----------------|-------------|
...
const ORG = 'truong-hai-truc'
const HCM = 'Hồ Chí Minh'
const bearer = (u: TestUser) => ({ Authorization: `Bearer ${u.token}` })

/** Tin của thành viên org. `reach` là thứ DUY NHẤT khác nhau giữa hai ca đầu. */
async function postAsMember(title: string, reach: 'members' | 'marketplace') {
...
beforeAll(async () => {
  ...
  await postAsMember('Tin NỘI BỘ của trường', 'members')
  await postAsMember('Tin CÔNG KHAI từ trường', 'marketplace')
 ...
 * `X-Org-Id`, nên chúng chỉ chứng minh scope mặc định là đúng.
 *
 ...
 * ra và tin `org_internal` chảy về cho người chỉ cần BIẾT ID — mà id nằm trong mọi link
 * chia sẻ. Đo trên dữ liệu thật trước khi sửa: 4/50 tin trả về là tin nội bộ.
 */
describe('Gửi X-Org-Id KHÔNG mở được nhánh org cho người ngoài', () => {
  it('người ngoài đã đăng nhập, gửi id của trường → vẫn KHÔNG thấy tin nội bộ', async () => {
    const titles = await titlesSeenBy(outsider, { 'X-Org-Id': orgIdOf(ORG) })
    expect(titles).not.toContain('Tin NỘI BỘ của trường')
    // Vẫn phải thấy tin công khai — siết quá tay thì gãy cả trục danh mục.
    expect(titles).toContain('Tin CÔNG KHAI từ trường')
  })

  it('khách CHƯA đăng nhập, gửi id của trường → vẫn KHÔNG thấy tin nội bộ', async () => {
    const res = await request(app)
      .get('/api/v1/listings')
  ...
  it('THÀNH VIÊN gửi id thì VẪN thấy tin nội bộ — chốt không siết quá tay', async () => {
    const titles = await titlesSeenBy(member, { 'X-Org-Id': orgIdOf(ORG) })
   ...
   * master trắng trơn.
   */
  it('MASTER gửi id vẫn đọc được tin nội bộ để duyệt', async () => {
    const titles = await titlesSeenBy(master, { 'X-Org-Id': orgIdOf(ORG) })
    expect(titles).toContain('Tin NỘI BỘ của trường')
  })
})

/*
 * `?reach=` — thứ mục "TIN TRONG NHÓM" ở hồ sơ nhóm cần.
 *
 ...
 */
describe('?reach= thu hẹp về đúng tin của nhóm', () => {
  it('org_internal cho thành viên → chỉ tin nội bộ, không lẫn tin công khai', async () => {
    const res = await request(app)
      .get('/api/v1/listings?reach=members&reach=group_open')
      .set({ ...bearer(member), 'X-Org-Id': orgIdOf(ORG) })
      .expect(200)
    const titles = res.body.data.map((l: { title: string }) => l.title)
  ...
  it('org_internal cho người ngoài → RỖNG, bộ lọc không thành cửa hậu', async () => {
    const res = await request(app)
      .get('/api/v1/listings?reach=members&reach=group_open')
      .set({ ...bearer(outsider), 'X-Org-Id': orgIdOf(ORG) })
      .expect(200)
...
describe('Người ngoài xem TIN CỦA MỘT NGƯỜI (?seller=)', () => {
  ...
  /** Gộp hai cửa hậu đã biết: id của nhóm + bộ lọc người bán. Cả hai cùng lúc vẫn phải đóng. */
  it('kèm X-Org-Id của nhóm cũng không mở ra được', async () => {
    const titles = await titlesOfSeller(member.id, outsider, { 'X-Org-Id': orgIdOf(ORG) })
    expect(titles).not.toContain('Tin NỘI BỘ của trường')
  })

  it('người ngoài cố lọc ?reach=members trên hồ sơ người khác → RỖNG', async () => {
    const res = await request(app)
      .get('/api/v1/listings')
      .query({ seller: member.id, reach: 'members' })
      .set(bearer(outsider))
      .expect(200)
  ...
  it('THÀNH VIÊN mở hồ sơ của chính người đó thì thấy CẢ HAI', async () => {
    const titles = await titlesOfSeller(member.id, member, { 'X-Org-Id': orgIdOf(ORG) })
